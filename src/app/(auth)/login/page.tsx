...
export default function LoginPage() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async (data) => {
      // Setup base auth state
      login(
        data.user, 
        null, // Workspace will be fetched by /me
      ...
      try {
        // Fetch full profile (which could contain workspace context)
        const profileResponse = await authService.getCurrentUser();
        useAuthStore.getState().updateUser(profileResponse);
      } catch (e) {
        console.error("Failed to fetch full profile post-login", e);
      }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Button variant="outline" type="button" className="w-full" asChild>
        <Link href="/auth/callback/google">
