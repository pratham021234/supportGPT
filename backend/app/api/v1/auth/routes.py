...
from app.schemas import RegisterRequest, LoginRequest, AuthResponse, ForgotPasswordRequest, ResetPasswordRequest, RefreshTokenRequest, UserResponse
...
@router.post("/login", response_model=Dict[str, Any], dependencies=[Depends(RateLimiter(times=5, seconds=60))])
async def login(request: Request, login_data: LoginRequest, db: AsyncSession = Depends(get_db)):
    ip_address = request.client.host if request.client else None
    user_agent = request.headers.get("user-agent")
    access_token, refresh_token, user = await auth_service.login(db, login_data, ip_address, user_agent)
...
async def refresh_token(request: Request, refresh_data: RefreshTokenRequest, db: AsyncSession = Depends(get_db)):
    ip_address = request.client.host if request.client else None
    user_agent = request.headers.get("user-agent")
    access_token, refresh_token = await auth_service.refresh(db, refresh_data.refresh_token, ip_address, user_agent)
    return {
        "success": True,
...
async def logout_all(current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
...
async def get_admin_analytics(current_user: User = Depends(get_current_active_user)):
    return {
        "success": True,
